...
- **Extension mechanism, web-facing** — the internal extension points gained a declarative contract, so an extension describes what it offers and zs-config renders and runs it. Extensions ship no markup and no JavaScript; the surface is administered from Admin, access is granted per extension to users or groups, and removal cleans up what the extension created.
- **Groups as a first-class concept** — groups were a SCIM-only detail: the IdP pushed them, they mapped to a role, and that was the whole story. `scim_groups` becomes `user_groups` with a source marker, so an admin can create a group here and pick its members by hand while SCIM keeps owning the ones it pushes; a SCIM sync replaces only its own rows, so a hand-added member survives it. Groups grant tenants as well as roles, and `group_service.effective_tenant_ids()` is the single place the union of direct and group grants is computed, so the two enforcement points cannot disagree. A new admin Groups page manages membership, role mapping, and tenant grants in one place.
- **Effective roles with session role assumption** — a group's mapped role no longer overwrites the user's own. It adds to the set of roles the account may assume, which fixes a locally created admin being silently demoted on joining a user-mapped group, and lets one account hold both roles without them fighting over a single column. Only one role is ever live: the JWT carries the active role in `role` and the whole set in `roles`, sessions start at least privilege, and `POST /auth/assume-role` switches and reissues both tokens so the choice survives a refresh. A role revoked mid-session falls back down rather than being honoured from a stale claim. The sidebar grows a role picker for accounts with more than one.
- **ZIA snapshot restore in the web API** — restore existed only in the TUI even though `ZIAPushService` already had every piece it needs, and that path was about to become unreachable. Adds preview and restore endpoints that stream progress over the existing job store, mirroring the TUI flow: classify, push creates and updates, verify and optionally remediate, delete resources absent from the snapshot, activate, verify the deletes, audit. `delete_extras` defaults to true here — a restore that leaves resources created since the snapshot in place is not a restore — and the preview exists so the UI can show that set before the user commits. Distinct from Apply Snapshot, which pushes one tenant's snapshot onto another and never deletes.
- **Staged configuration changes** — `zpa_resources` and `zia_resources` have long carried `source` and `candidate_status` columns with nothing implementing them. `services/candidate_service.py` provides that lifecycle, product-neutral across ZPA and ZIA: ingest → pending → accepted → push → promote → live row, with reject and discard alongside. Ingest takes the snapshot wire shape, so a producer hands over what it generated without a bespoke format, and re-ingesting updates rather than duplicates. Promote defers to the tenant: if an import landed the real object between the push and the promote, the staged row is dropped and the imported row stands.
- **Staged-to-push baseline adapter** — `services/candidate_baseline.py` is the handoff between staged rows and the push engines, kept separate so `candidate_service` stays free of any dependency on them. It answers two questions before any API call is made: which rows the engine can actually write (the rest move to `excluded` with a reason, rather than being silently dropped), and whether anything references a row that is not coming along, since a partial baseline can leave a dangling synthetic id.
- **ZPA push engine as a service** — the only ZPA write engine lived as ~330 lines of nested closures inside the restore handler, so nothing outside it could reach the engine: no dry run, no rollback, no way to test it without a request. It is now `services/zpa_push_service.py`, mirroring `ZIAPushService`'s classify/execute contract, which unlocks a merge mode that matches by name and has no delete path reachable from it, and classification without writes exposed as a restore preview. The duplicate capability sets in the snapshot diff endpoint fold into the service constants so the preview cannot drift from the executor.

### Changed

- **Administering an extension and using one are separate roles** — an admin installs, grants, and removes; they do not run, exactly as they cannot reach Templates or Scheduled Tasks. An account that needs both holds both roles and switches with the role picker.

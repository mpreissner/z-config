...
def create_user(tenant: str, body: dict, user: AuthUser = Depends(require_auth)):
    svc = _get_service(tenant, user)
    return svc.create_user(body)


@router.put("/{tenant}/users/{user_id}")
def update_user(tenant: str, user_id: str, body: dict, user: AuthUser = Depends(require_auth)):
    svc = _get_service(tenant, user)
...
def delete_user(tenant: str, user_id: str, body: dict = None, user: AuthUser = Depends(require_auth)):
    svc = _get_service(tenant, user)
...
def reset_password(tenant: str, user_id: str, user: AuthUser = Depends(require_auth)):
    svc = _get_service(tenant, user)
...
def set_password(tenant: str, user_id: str, body: SetPasswordRequest, user: AuthUser = Depends(require_auth)):
    svc = _get_service(tenant, user)
...
def skip_mfa(tenant: str, user_id: str, body: SkipMfaRequest, user: AuthUser = Depends(require_auth)):
    svc = _get_service(tenant, user)
...
def get_api_client_secrets(tenant: str, client_id: str, user: AuthUser = Depends(require_auth)):
    svc = _get_service(tenant, user)
...
def delete_api_client(tenant: str, client_id: str, user: AuthUser = Depends(require_auth)):
    svc = _get_service(tenant, user)
